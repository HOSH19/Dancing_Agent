...
from envs.walk_env import SWING_W, UPRIGHT_W
from envs.arm_env import ARM_W
...
def _step(envs, agent, buffer, device, obs, totals):
    ...
    totals["clearance"] += float(infos.get("reward_clearance", np.zeros(n)).sum()) * SWING_W
    totals["upright"]   += float(infos.get("reward_upright",   np.zeros(n)).sum()) * UPRIGHT_W
    totals["arm"]       += float(infos.get("reward_arm",       np.zeros(n)).sum()) * ARM_W
...
def collect(envs, agent, buffer, device, obs, terms, truncs):
    totals = dict(reward=0.0, forward=0.0, clearance=0.0, upright=0.0, arm=0.0)
    n_steps, n_done = 0, 0
